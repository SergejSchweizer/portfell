...
def test_projected_workflow_reader_records_bounded_non_response_metrics() -> None:
    payload: dict[str, object] = {"schema_version": 1, "stages": {}}
    metrics = WorkflowReadMetrics()
    statements = iter((4, 5))
    clock = iter((10.0, 10.125))
    reader = PostgresProjectedWorkflowReader(
        _Projections((payload, "etag-1"), None),  # type: ignore[arg-type]
        metrics=metrics,
        statement_count=lambda: next(statements),
        clock=lambda: next(clock),
    )

    reader("user-1", None)

    assert metrics.observations[0].route == "workflow"
    assert metrics.observations[0].statement_count == 1
    assert metrics.observations[0].response_bytes > 0
    assert metrics.observations[0].shared_file_reads == 0
    assert metrics.observations[0].elapsed_seconds == 0.125
