...
def _split(
    *, split_id: str, candidate_id: str, train_end: str, test_start: str, test_end: str, value: float
) -> ValidationSplit:
...
def test_structural_evidence_reuses_exact_oos_metrics_and_is_deterministic() -> None:
    ...
    assert all(row.effective_pca_risk_drivers is None or row.effective_pca_risk_drivers >= 1.0 for row in first)
    assert all(row.largest_pca_risk_share is None or 0.0 <= row.largest_pca_risk_share <= 1.0 for row in first)
    assert all(
