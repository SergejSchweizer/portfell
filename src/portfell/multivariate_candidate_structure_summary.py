...
def summarize_candidate_pca_risk(risk: CandidatePcaRisk) -> CandidatePcaRiskSummary:
    if not risk.available or risk.portfolio_variance is None:
        return _unavailable(risk, risk.availability_reasons or ("candidate_pca_non_positive_variance",))
...
def _unavailable(
    risk: CandidatePcaRisk, reasons: tuple[str, ...]
) -> CandidatePcaRiskSummary:
    return CandidatePcaRiskSummary(
        risk.candidate_id,
        risk.method,
        None,
