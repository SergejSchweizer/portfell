...
"""Dash callback registration for Bivariate Statistics."""
...
def _control(row: Mapping[str, object]) -> StatisticsRunControl:
    ...
    return StatisticsRunControl(
        stage_id=WorkflowId.BIVARIATE_STATISTICS.value,
        status=status,
...
def register_bivariate_callbacks(app: Dash, gateway: DashResearchGateway) -> None:
    """Register explicit start/poll behavior over persisted upstream selection identity."""
    ...
    def update_bivariate(
        ...
        if ctx.triggered_id == component_id(BIVARIATE_NAMESPACE, "compute") and n_clicks:
            ...
            upstream = _mapping(page.get("input")).get("univariate_selection_id")
            if not isinstance(upstream, str) or not upstream:
                return None, "unavailable", "univariate_selection_unavailable", True
            status_row = gateway.start_run(
                project_slug=project_slug,
                stage_id=WorkflowId.BIVARIATE_STATISTICS,
                command_key=start_command_key(
                    project_slug=project_slug,
                    univariate_revision=upstream,
                ),
                settings={},
            )
        else:
