...
def candidate_return_risk_figure(
    points: Iterable[PortfolioCandidatePoint],
) -> dict[str, object]:
    ...
    available = [
        ...
        if point.annualized_oos_return is not None
        and point.oos_annualized_volatility is not None
    ]
    unavailable = [
        point.configuration_id
        for point in ordered
        if point.annualized_oos_return is None
        or point.oos_annualized_volatility is None
    ]
    objectives = sorted({point.objective for point in ordered})
    winners = [point.configuration_id for point in ordered if point.winner]
    return {
        "data": [
            {
                "type": "scattergl",
                ...
                "hovertemplate": (
                    "%{text}<br>OOS volatility=%{x}<br>OOS return=%{y}<br>"
                    "method=%{customdata[0]}<br>risk=%{customdata[1]}<br>"
                    "objective=%{customdata[2]}<br>winner=%{customdata[4]}<extra></extra>"
                ),
            }
        ],
        "layout": {
            ...
            "meta": {
                ...
                "winner_configuration_ids": winners,
                "unavailable_configuration_ids": unavailable,
            },
