...
def _projected_gradient_descent(
    *,
    ...
    for _iteration in range(1, max_iterations + 1):
        ...
        step = min(step * 2.0, maximum_step)
        while True:
...
def solve_minimum_variance(
    ...
    *,
    ...
    # The gradient magnitude follows the covariance scale. Daily covariance
    # matrices are commonly around 1e-4, where a unit step barely moves from
    # equal weight even after many iterations. This row-sum bound is a safe
    # inverse Lipschitz step for the quadratic objective w^T Sigma w.
    largest_row_sum = max((sum(abs(value) for value in row) for row in matrix), default=0.0)
    maximum_step = 1.0 if largest_row_sum == 0.0 else 1.0 / (2.0 * largest_row_sum)
