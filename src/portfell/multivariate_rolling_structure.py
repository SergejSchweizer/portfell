    ...
    *,
    ...
    for end_index in reversed(endpoints):
        ...
        try:
            ...
            output.append(_unavailable_row(window_dates, "rolling_structure_risk_model_unavailable"))
            continue
        ...
        output.append(
            RollingStructureRow(
                ...
                correlation_effective_rank=(correlation.effective_rank if correlation is not None else None),
...
def _common_dates(
    ...
    dates_by_listing: dict[MultivariateListingKey, set[str]] = {listing: set() for listing in listings}
...
def _unavailable_row(dates: tuple[str, ...], reason: str) -> RollingStructureRow:
    return RollingStructureRow(dates[0], dates[-1], len(dates), None, None, None, None, None, (reason,))
