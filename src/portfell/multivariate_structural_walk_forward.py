...
from portfell.multivariate_risk_model import RISK_MODEL_ARTIFACT_CONTRACT, build_multivariate_risk_model
...
def build_structural_walk_forward_evidence(
    *,
    ...
    for training_rows, refitted in zip(training_windows, refitted_candidate_sets, strict=True):
        train_dates = tuple(sorted({str(row.get("date", "")) for row in training_rows if row.get("date")}))
        if not train_dates:
        ...
        refitted_by_identity = {(candidate.candidate_id, candidate.method): candidate for candidate in refitted}
        ...
        for split in window_splits:
            ...
            evidence.append(
                StructuralWalkForwardEvidence(
                    evidence_id=stable_contract_id("structural_walk_forward_evidence", identity_payload),
                    split_id=split.split_id,
                    candidate_id=split.candidate_id,
                    method=split.method,
                    train_start=split.train_start,
    ...
    return tuple(sorted(evidence, key=lambda row: (row.test_start, row.method, row.candidate_id, row.split_id)))
