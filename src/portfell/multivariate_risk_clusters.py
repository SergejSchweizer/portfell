...
def build_hierarchical_risk_clusters(
    *,
    ...
    distances = tuple(
        tuple(sqrt(max(0.0, (1.0 - correlation[left][right]) / 2.0)) for right in range(len(listings)))
        for left in range(len(listings))
    )
    clusters: list[tuple[int, ...]] = [(index,) for index in range(len(listings))]
    while len(clusters) > 1:
        candidates: list[tuple[float, tuple[MultivariateListingKey, ...], tuple[MultivariateListingKey, ...], int, int]] = []
        ...
        if distance > CLUSTER_DISTANCE_CUT:
            break
        merged = tuple(sorted((*clusters[left], *clusters[right]), key=lambda index: listings[index]))
    ...
    canonical_clusters = sorted(clusters, key=lambda cluster: min(listings[index] for index in cluster))
