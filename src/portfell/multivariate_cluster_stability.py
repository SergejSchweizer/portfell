    ...
    *,
    ...
    for _ in range(BOOTSTRAP_REPLICATES):
        ...
        while len(sampled_indexes) < len(dates):
            ...
            sampled_indexes.extend((start + offset) % len(dates) for offset in range(BOOTSTRAP_BLOCK_LENGTH))
    ...
    for cluster_id in sorted(set(canonical_membership.values()), key=_cluster_sort_key):
        members = tuple(sorted(listing for listing, label in canonical_membership.items() if label == cluster_id))
        if len(members) < 2:
            cluster_rows.append(
                CanonicalClusterStability(cluster_id, None, None, ("cluster_stability_not_applicable_singleton",))
            )
            continue
        probabilities = tuple(
...
def _aligned_values(
    ...
    for row in return_rows:
        key = MultivariateListingKey(str(row.get("isin", "")), str(row.get("exchange", "")), str(row.get("code", "")))
...
def _unavailable(reason: str) -> ClusterBootstrapStability:
    return ClusterBootstrapStability((), (), BOOTSTRAP_REPLICATES, BOOTSTRAP_BLOCK_LENGTH, BOOTSTRAP_SEED, (reason,))
