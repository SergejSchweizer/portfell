...
def build_candidate_cluster_risk(
    *,
    ...
    for cluster_id in cluster_ids:
        indexes = tuple(
            index
            for index, listing in enumerate(risk_model.listings)
            if membership[listing] == cluster_id
        )
    ...
    if not isclose(
        sum(row.signed_variance_contribution for row in rows),
        portfolio_variance,
        rel_tol=1e-9,
        abs_tol=1e-12,
    ):
        return _unavailable(candidate, risk_model, "candidate_cluster_variance_mismatch")
    if not isclose(
        sum(row.signed_percent_variance for row in rows),
        1.0,
        rel_tol=1e-9,
        abs_tol=1e-12,
    ):
        return _unavailable(candidate, risk_model, "candidate_cluster_variance_mismatch")
    if not isclose(
        sum(row.gross_abs_risk_share for row in rows),
        1.0,
        rel_tol=1e-9,
        abs_tol=1e-12,
    ):
        return _unavailable(candidate, risk_model, "candidate_cluster_variance_mismatch")
    return CandidateClusterRisk(
        candidate.candidate_id,
        candidate.method,
        risk_model.risk_model_id,
        tuple(rows),
