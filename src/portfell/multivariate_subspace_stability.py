...
def build_adjacent_subspace_stability(
    *,
    ...
    if len(date_ends) < 2:
        return (
            SubspaceStabilityRow("", "", None, None, min(3, listing_count), ("subspace_stability_insufficient_windows",)),
        )
    if len(covariance_bases) != len(date_ends) or len(correlation_bases) != len(date_ends):
        return (
            SubspaceStabilityRow("", "", None, None, min(3, listing_count), ("subspace_stability_invalid_basis_series",)),
        )
    ...
    for index in range(1, len(date_ends)):
        ...
        reasons = () if covariance is not None and correlation is not None else ("subspace_stability_unavailable",)
        rows.append(
            SubspaceStabilityRow(
                date_ends[index - 1], date_ends[index], covariance, correlation, component_count, reasons
            )
        )
    return tuple(rows)
