...
from portfell.multivariate_risk_clusters import RiskClusterDiagnostics, build_hierarchical_risk_clusters
...
def build_structure_v2_documents(
    *,
    ...
    signal = build_signal_component_diagnostics(return_rows=return_rows, listings=risk_model.listings)
    rolling = build_rolling_structure_diagnostics(return_rows=return_rows, listings=risk_model.listings)
    ...
    structure_id = stable_contract_id(
        "multivariate_structure_v2",
        {
            ...
            "parallel_analysis": {"replicates": 100, "seed": 41, "quantile": 0.95, "method": "higher"},
            ...
            "items": [
                {
                    ...
                    "correlation_dominant_component_share": row.correlation_dominant_component_share,
        ...
        candidate_rows.append(
            {
                ...
                "availability_reasons": sorted(
                    set((*pca_risk.availability_reasons, *summary.availability_reasons, *cluster_risk.availability_reasons))
                ),
    ...
    return StructureV2Documents(structure_id, candidate_structure_id, structure_document, candidate_document)
...
def _listing_row(listing: object) -> JsonRow:
    key = listing
    return {
        "isin": str(getattr(key, "isin")),
        "exchange": str(getattr(key, "exchange")),
        "code": str(getattr(key, "code")),
    }
