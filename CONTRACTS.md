...
- [Analytical Statistics Persistence](#analytical-statistics-persistence)
...
## Analytical Statistics Persistence

Univariate, Bivariate, and Multivariate results are persisted in the Portfell PostgreSQL
control-plane database as immutable analysis runs and analysis artifacts. The local market
snapshot is input data only; it is never the authoritative store for calculated statistics.

```text
local market snapshot -> computation -> PostgreSQL analysis_run + analysis_artifact
                                            |
                                            v
                                  UI reads persisted artifacts
```

Reopening a project therefore loads the last successful run from PostgreSQL. A calculation is
considered complete only after its run status is `succeeded` and all required artifacts have been
published. A failed or interrupted run remains non-authoritative and must not replace the last
successful result.
