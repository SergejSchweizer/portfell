...
- Pending result workspace: until a run completes, only the Overview and Portfolio Candidates tabs
  are visible. Their candidate and metric surfaces render their empty state until server-produced
  values are available.
- Complete result workspace: Overview, Risk Structure, Portfolio Candidates, Risk Contributions,
  Income Evidence, Performance, and Validation appear in the responsive statistics tab grid, with
  no horizontal tab scrolling. Every table retains its headers and the Performance chart keeps its
  labelled plot frame when its result data is unavailable.
...
- Running: the server-owned phase and completed/total units are displayed and polled every
  ...
  the server reports `running`. The full result workspace remains available with empty tables,
  candidate surfaces, and an empty performance plot until their persisted results arrive. Independent candidate optimizers and their Walk-Forward refits run
