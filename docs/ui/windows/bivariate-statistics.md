...
The header shows the server-owned Bivariate ISIN count from the Bivariate plan's pinned Univariate
selection, never the broader metadata or project listing count.
...
bounded results page. While that complete payload is loading, its aligned data period and shared
observation count use the matching completed Bivariate summary.
