...
The metadata action panel precedes the filter dropdowns in document order. Metadata refresh remains disabled without an entered or saved header key. All fields have visible labels, status changes use `aria-live`, and no filtering or ingestion business logic is implemented in the browser.
