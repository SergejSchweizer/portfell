...
Every workflow item also shows the project selection's server-owned quote coverage range in
`dd.MM.yyyy-dd.MM.yyyy` format when quote coverage exists. The range is derived only from quote
coverage records for the project's selected listings and uses the same font size as the status text.
