...
Consequences: Development changes should pass the local pre-commit hooks before integration, and repository changes should keep workflow, README commands, and backlog status aligned.
...
Consequences: Branch protection must not require a GitHub Actions `quality` status check unless a workflow is reintroduced. Contributors should run `uv run pre-commit run --all-files` locally before integration.
...
Consequences: Working copies should run the local quality checks before integration, commits must use Conventional Commit subjects, and rebasing onto current `main` must precede integration. Integration fails when test coverage is below 95%.
