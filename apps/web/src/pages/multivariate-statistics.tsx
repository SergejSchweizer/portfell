...
function PerformanceChart({ performance, alignedPeriod }: Readonly<{ performance: ApiMultivariatePerformance; alignedPeriod?: Readonly<{ date_start: string; date_end: string }> }>) {
  ...
  return <>
    <p className="performance-chart__legend">Cumulative returns for every input instrument and feasible portfolio method.</p>
    <div className="performance-chart" role="group" aria-label="Cumulative return comparison for all instruments and feasible portfolios. Hover or use arrow keys to inspect a date." tabIndex={0} onMouseLeave={() => setHoveredIndex(null)} onMouseMove={(event) => inspectAt(event.clientX, event.currentTarget.getBoundingClientRect())} onKeyDown={(event) => {
