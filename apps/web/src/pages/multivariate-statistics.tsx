...
function PerformanceChart({ performance, alignedPeriod }: Readonly<{ performance: ApiMultivariatePerformance; alignedPeriod?: Readonly<{ date_start: string; date_end: string }> }>) {
  ...
  if (allValues.length === 0) return <div className="performance-chart performance-chart--empty" role="img" aria-label="Relative cumulative monthly return comparison awaiting multivariate results"><svg viewBox="0 0 800 280" preserveAspectRatio="none" aria-hidden="true"><text className="performance-chart__axis-title" x="12" y="130" textAnchor="middle" transform="rotate(-90 12 130)">Cumulative relative gain (%)</text><line className="performance-chart__zero" x1="20" x2="780" y1="240" y2="240" /></svg><p role="status">Performance results will appear when computation is complete.</p></div>;
  ...
  const monthGridLines = timeline.map((item) => {
    const time = Date.parse(item.date);
    return { date: item.date, x: 20 + (time - start) / Math.max(1, end - start) * 760 };
  });
  ...
  return <>
    ...
    {hasVisibleSeries ? <div className="performance-chart" role="group" aria-label="Relative cumulative monthly return comparison for visible instruments and feasible portfolios. Hover or use arrow keys to inspect a month." tabIndex={0} onMouseLeave={() => setHoveredIndex(null)} onMouseMove={(event) => inspectAt(event.clientX, event.currentTarget.getBoundingClientRect())} onKeyDown={(event) => {
      ...
      <svg viewBox="0 0 800 280" preserveAspectRatio="none" aria-hidden="true">
        ...
        {monthGridLines.map((item) => <line className="performance-chart__month-grid" key={item.date} x1={item.x} x2={item.x} y1="20" y2="240" />)}
...
const emptyPerformance: ApiMultivariatePerformance = {
  instrument_series: [],
  portfolio_series: [],
  period_returns: [],
};
...
export function MultivariateStatisticsPage() {
  ...
  const visibleTabs = run?.status === "complete"
    ? tabs
    : tabs.filter((tab) => tab.id === "overview" || tab.id === "portfolio-candidates");
  return <section className="multivariate-statistics-page" data-route="multivariate-statistics-page">
    ...
    <Panel title="Multivariate results">
      {run?.status === "running" && candidates ? <p role="status">Previous completed portfolio candidates are shown while the new calculation runs.</p> : null}
      <div className="statistics-tabs" role="tablist" aria-label="Multivariate statistics views">{visibleTabs.map((tab) => <button key={tab.id} role="tab" aria-selected={activeTab === tab.id} onClick={() => setActiveTab(tab.id)}>{tab.label}</button>)}</div>
      {activeTab === "overview" && <><PerformanceChart performance={performance ?? emptyPerformance} alignedPeriod={summary?.aligned_period} /><PortfolioOverviewMetrics candidates={candidates?.items ?? []} />{!candidates && <p role="status">Portfolio metrics will appear when computation is complete.</p>}{summary?.availability_reasons?.length ? <p role="status">Unavailable evidence: {summary.availability_reasons.join(", ")}</p> : null}{historyRequirement(summary?.availability_reasons) ? <p role="status">{historyRequirement(summary?.availability_reasons)}</p> : null}</>}
