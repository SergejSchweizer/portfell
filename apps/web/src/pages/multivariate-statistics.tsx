...
export function MultivariateStatisticsPage() {
  ...
  return <section className="multivariate-statistics-page" data-route="multivariate-statistics-page">
    ...
    {run?.status === "complete" && <Panel title="Multivariate results">
      ...
      {activeTab === "income-evidence" && <><p>All income values are gross historical observations. Net, sustainable, tax, cost, and genuine-NAV claims remain unavailable unless a verified source is present.</p><table><caption>Monthly-distribution evidence</caption><thead><tr><th>Listing</th><th>Observed months</th><th>Coverage</th><th>Gross TTM yield</th><th>Trend</th><th>Cuts</th><th>Total return</th><th>NAV erosion</th><th>Warnings</th></tr></thead><tbody>{income?.items.map((item) => <tr key={`${item.isin}:${item.exchange}:${item.code}`}><td>{item.code}.{item.exchange}</td><td>{item.observed_month_count}</td><td>{percent(item.observed_payment_coverage)}</td><td>{percent(item.gross_ttm_distribution_yield)}</td><td>{number(item.distribution_trend)}</td><td>{item.cut_count ?? "Unavailable"}</td><td>{percent(item.total_return)}</td><td>{percent(item.nav_erosion)}</td><td>{[...item.warnings, ...item.availability_reasons].join(", ") || "None"}</td></tr>)}</tbody></table></>}
