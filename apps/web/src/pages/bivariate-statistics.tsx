...
function PairMatrix({
  ...
  return <div className="bivariate-statistic__results">
    {matrix === null ? <p className="status-line">Compute bivariate statistics to populate the {title.toLowerCase()} matrix.</p> : matrix.labels.length > 0 ? <>
      <p className="bivariate-statistic__matrix-caption">Daily log-return {title.toLowerCase()} matrix · {dataPeriod(matrix.date_start, matrix.date_end)} · {matrix.observation_count.toLocaleString()} shared observations</p>
