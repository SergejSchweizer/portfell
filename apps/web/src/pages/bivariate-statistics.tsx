...
export function BivariateStatisticsPage() {
  ...
  const selectionId = pageView.status === "ready"
    ? pageView.data.input.univariate_selection_id
    : null;
  ...
  const pairPlan = useQuery({
    queryKey: ["bivariate-plan", selectionId],
    queryFn: () => bivariateStatisticsApi.plan({ univariate_selection_id: selectionId! }),
  ...
  const alignedPeriodStart = loadedTailRiskScatter?.date_start ?? loadedSummary?.date_start;
  const alignedPeriodEnd = loadedTailRiskScatter?.date_end ?? loadedSummary?.date_end;
  const alignedObservationCount = loadedTailRiskScatter?.observation_count ?? loadedSummary?.observation_count;
  ...
  const bivariateIsinCount = pairPlan.data?.selected_listing_count;
  return (<>
    {pageView.refreshing ? <LoadingIndicator label="Loading selected project" compact /> : null}
    <Panel title={<><span>Bivariate Statistics</span>{bivariateIsinCount !== undefined && <small className="bivariate-statistics__isin-count">{bivariateIsinCount.toLocaleString()} ISINs</small>}</>}>
      ...
      {pageViewCompleted && <section className="bivariate-statistic" aria-labelledby="pairwise-dependence-title">
        ...
        <div className="bivariate-statistic__facts">
          ...
          {activePairwiseMetric === "tail_risk_scatter" ? <>
            ...
            <dl>
              <div><dt>Aligned data period</dt><dd>{dataPeriod(alignedPeriodStart, alignedPeriodEnd)}</dd></div>
              <div><dt>ISIN pairs plotted</dt><dd>{loadedTailRiskScatter?.pair_count.toLocaleString() ?? "—"}</dd></div>
              <div><dt>Shared observations</dt><dd>{alignedObservationCount?.toLocaleString() ?? "—"}</dd></div>
              <div><dt>Tail-dependence median</dt><dd>{percent(loadedTailRiskScatter?.tail_dependence_median)}</dd></div>
              <div><dt>Co-exceedance median</dt><dd>{percent(loadedTailRiskScatter?.coexceedance_rate_median)}</dd></div>
              <div><dt>Best-diversifier quadrant</dt><dd>{scatterDiagnostics ? `${scatterDiagnostics.best_diversifiers.toLocaleString()} pairs (${(scatterDiagnostics.best_diversifiers / Math.max(1, loadedTailRiskScatter?.pair_count ?? 0) * 100).toFixed(1)}%)` : "—"}</dd></div>
              <div><dt>Tail-risk concentration quadrant</dt><dd>{scatterDiagnostics ? `${scatterDiagnostics.tail_concentration.toLocaleString()} pairs (${(scatterDiagnostics.tail_concentration / Math.max(1, loadedTailRiskScatter?.pair_count ?? 0) * 100).toFixed(1)}%)` : "—"}</dd></div>
